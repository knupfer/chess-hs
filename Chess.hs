    ...
    a = M.foldrWithKey
        (\k list acc@(s,_) ->
          ...
          then (fst $ scoreFold k list, (k,snd $ scoreFold k list))
                  ...
                  && (k,head list) `notElem` bad
               then (s,(k,head list))
               else acc)
        ((500,500),((1,2),(5,5))) g
    scoreFold k = foldr (\l acc@(s,_) ->
                          (if (score k l < s) && (k,l) `notElem` bad
                           then (score k l, l)
                           else acc))
                  ((200,200),(1,2))
    g = M.filter (not . null)
...
validateMove pl fs (oldPos,newPos)
  | oldPos `M.notMember` fs = Left "Your first coordinate isn't a piece."
  | pl /= (getColor . fromJust $ M.lookup oldPos fs) = Left "Wrong Color."
  | newPos `S.member` possibleMoves oldPos fs
...
possibleMoves (x,y) fs = g $ case piece of
  ...
  where
    ...
    l dc dr   = (\s -> s ++ [maximum s + 1] ++ [minimum s - 1]) $ 0 :
              concatMap (takeWhile (\d -> (dc x d,dr y d) `M.notMember` fs))
              [[1..8],[-1,-2..(-8)]]
    pawn f r = [z | z <- [(x+1,f y 1),(x-1,f y 1)], z `M.member` fs] ++
               (if (x,f y 1) `M.notMember` fs
                then (x,f y 1):[(x,f y 2)| y == r && (x,f y 2) `M.notMember` fs]
                else [])
